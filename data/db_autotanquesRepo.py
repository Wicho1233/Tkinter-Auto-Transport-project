...
def crear_tabla_si_no_existe():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS taraAdaptacion (
            tr INTEGER,   
    ...
    ''')
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS taraDivision (
            tr INTEGER,   
            numero INTEGER,
            nombreDivision TEXT,
            mantenimiento TEXT,
...
def insertar_mantenimiento_adaptacion(tr, nombreAdaptacion, mantenimiento, fechaMant, kiloActu, kiloMant, costo, estado):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO taraAdaptacion (tr, nombreAdaptacion, mantenimiento, fechaMant, kiloActu, kiloMant, costo, estado)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ''', (tr, nombreAdaptacion, mantenimiento, fechaMant, kiloActu, kiloMant, costo, estado))
    conn.commit()
    conn.close()
...
def insertar_mantenimiento_division(tr, numero, nombreDivision, mantenimiento, fechaMant, kiloActu, kiloMant, costo, estado):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO taraDivision (tr, numero, nombreDivision, mantenimiento, fechaMant, kiloActu, kiloMant, costo, estado)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (tr, numero, nombreDivision, mantenimiento, fechaMant, kiloActu, kiloMant, costo, estado))
    conn.commit()
    conn.close()
